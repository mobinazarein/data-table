    ...
    <v-row no-gutters>
      <v-col cols="12" md="12" class="mt-5">
        ...
        <v-row class="justify-space-between align-center px-4 mt-4">
          <!-- نمایش تعداد موارد -->
          <span class="text-gray"> {{ startIndex }} - {{ endIndex }} از {{ totalItems }} مورد </span>

          <!-- صفحه‌بندی -->
          <v-pagination v-model="page" :length="totalPages" rounded color="primary"></v-pagination>

          <!-- انتخاب تعداد آیتم‌های هر صفحه -->
          <v-select v-model="itemsPerPage" :items="[10, 20, 30, 50]" density="compact" variant="outlined"
            class="custom-select" label="نمایش در صفحه"></v-select>
        </v-row>
