...
// تعیین مقدار تب فعال بر اساس مسیر فعلی
const tab = computed(() => {
    const currentTab = pageData.items.find((item) => item.link === route.name);
    return currentTab ? currentTab.value : 0; // اگر مسیر ناشناخته بود، محاسبه (0) انتخاب شود
});
</script>
