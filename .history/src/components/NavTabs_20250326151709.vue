...
const tab = ref(0); // Initialize the first tab as selected
const pageData = {
    items: [
...
// Apply custom style to the initially selected tab
onMounted(() => {
    applyCustomStyle(tab.value);
});
...
const applyCustomStyle = (selectedTab) => {
    ...
    tabs.forEach((tab, index) => {
        if (index === selectedTab) {
            ...
            tab.style.background = ""; // Reset style for other tabs
            tab.style.color = "";
