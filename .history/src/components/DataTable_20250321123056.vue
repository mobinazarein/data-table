  ...
  <v-container fluid>
    ...
    <v-row no-gutters>
      <v-col cols="12" md="12" class="mt-5">
        <v-data-table :headers="pageData.headers" :items="pageData.items" :items-per-page="10" hide-default-footer>
        ...
        <v-row class="justify-space-between align-center px-4 mt-4">
          <div class="d-flex align-center">
            <p>نمایش در هر صفحه</p>
            <v-select v-model="itemsPerPage" :items="[10, 20, 30, 50]" density="compact" variant="outlined"
              rounded="pill" class="custom-select mr-3" hide-details></v-select>
          </div>
          <!-- نمایش تعداد موارد -->

          <!-- صفحه‌بندی -->
          <v-pagination v-model="page" :length="totalPages" rounded density="comfortable" color="primary"
            class="custom-pagination"></v-pagination>

          <!-- انتخاب تعداد آیتم‌های نمایش داده‌شده -->
...
const totalItems = ref(19); // تعداد کل آیتم‌ها
