            ...
            <div class="d-flex justify-space-between gap-8">
              ...
              <div class="d-flex">
                ...
                <v-btn v-for="(button, index) in mobileFilterButtons" :key="index" variant="outlined" color="gray"
                  class="ml-2 rounded-lg filter-btn"
                  :style="{ maxWidth: button.width + 'px', minWidth: button.width + 'px', fontSize: '12px', borderRadius: button.borderRadius + 'px' }">
                  {{ button.label }}
                </v-btn>
              </div>
            </div>
...
const props = defineProps({
  ...
  mobileFilterButtons: {
    type: Array,
    default: () => [
      { label: "همه", minWidth: 44, maxWidth: 44, borderRadius: '62px' },
      { label: "ابطال شده", minWidth: 74, maxWidth: 80, borderRadius: 42 },
      { label: "وصول شده", minWidth: 77, maxWidth: 89, borderRadius: 42 }
    ]
  }
});
const page = ref(1);
