      ...
      <v-col cols="12" md="10" class="d-flex align-center  justify-start">
        <div class="d-flex align-center gap-24 justify-start">
          <div class="d-flex align-center text-nowrap ">
            <PauseIcon />
            <h2 class="mr-3 ml-5 text-nowrap " style="white-space: nowrap;">سوابق فیش</h2>
          </div>
          <PaginationIcon class="mx-1" />
          <EyeIcon class="mx-1" />
          <InvoiceIcon class="mx-1 ml-2" />
          <CircleCross class="mx-1 ml-2" />
          ...
          <v-btn variant="outlined" color="gray" class="rounded-lg ml-2 filter-btn"
            style="max-width: 48px;min-width: 48px;font-size: 14px !important;">همه</v-btn>
          <v-btn variant="outlined" color="gray" class="ml-2 rounded-lg filter-btn"
            style="max-width: 87px;min-width: 87px;font-size: 14px !important;">ابطال شده</v-btn>
          <v-btn variant="outlined" color="gray" class="ml-2 rounded-lg filter-btn px-3"
            style="max-width: 92px;min-width: 92px;font-size: 14px !important;">وصول شده</v-btn>
        </div>
      </v-col>
            ...
            <div class="d-flex justify-space-between gap-8">
              ...
              <div class="d-flex">
                <v-btn variant="outlined"
                  ...
                  color="gray" class="rounded-xl px-4 ml-2 filter-btn">همه</v-btn>
                <v-btn variant="outlined"
                  style="max-width: 80px !important;min-width:74px !important;height: 40px !important;font-size: 12px;font-weight: 400;border-radius: 42px !important;"
                  color="gray" class="ml-2 rounded-xl px-4 filter-btn">ابطال شده</v-btn>
                <v-btn variant="outlined" color="gray" class=" rounded-xl px-4 filter-btn ml-1"
                  style="max-width: 89px !important;min-width:77px !important;height: 40px !important;font-size: 12px;font-weight: 400;border-radius: 42px !important;">وصول
                  شده</v-btn>
              </div>
            </div>
          </v-col>
      ...
      <v-col cols="12" class="d-flex justify-center">
        <div class="d-flex flex-column ">
          <div class="gap-8 d-flex justify-space-between flex-wrap">
            ...
            <ExcelIcon iconColor="#767E8A" class="mx-1 mt-3 mr-3  ml-5" />
          </div>
          <v-text-field width="341px" variant="outlined" hide-details class="custom-text-field mx-2"
            label="جستجو کنید..." prepend-inner-icon="mdi-magnify" v-if="showSearchBtn" />
        </div>
      </v-col>
    </v-row>
    <v-row no-gutters class="px-1">
      <v-col cols="12" md="12" class="mt-5">
        <v-data-table :headers="pageData.headers" :items="pageData.items" :items-per-page="props.itemsPerPage"
          hide-default-footer>
...
<template v-slot:item.documentNumber="{ item }">
            <v-chip color="blue" class="ma-1 "
              style="width: 150px;height: 22px !important; border-radius: 8px;padding: 0;font-size: 12px;">
...
<template v-slot:item.receiptType="{ item }">
          ...
          <v-data-table :headers="pageData.headers" :items="pageData.items" :items-per-page="props.itemsPerPage"
            hide-default-footer>
            <template v-for="(column, index) in props.customStyles" #[`item.${column.name}`]="{ item }">
              <v-chip:style="{ backgroundColor: column.bgColor, color: column.textColor, height: '22px' ,
                fontSize: '12px' }" class="font-weight-bold">
                {{ item[column.name] }}
                </v-chip>
            </template>
          </v-data-table>
...
.custom-pagination .v-pagination__prev:focus,
.custom-pagination .v-pagination__next:focus,
.custom-pagination .v-pagination__prev.v-pagination__item--active,
.custom-pagination .v-pagination__next.v-pagination__item--active {
  border: 1px solid #148976 !important;
  color: #148976 !important;
}
