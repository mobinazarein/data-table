...
watch(
    () => route.name,
    (newRoute) => {
        nextTick(() => {  // اطمینان از اینکه تغییرات اعمال شده است
            if (newRoute) {  // بررسی می‌کنیم که `newRoute` مقدار داشته باشد
                const currentTab = pageData.items.find((item) => item.link === newRoute);
                tab.value = currentTab ? currentTab.value : 0;
            } else {
                // در صورت نداشتن `newRoute` به طور پیش‌فرض بر روی تب اول برو
                tab.value = 0;
            }
        });
    },
