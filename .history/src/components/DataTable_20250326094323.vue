...
<template v-slot:item.receiptType="{ item }">
        ...
        <v-data-table :headers="pageData.headers" :items="pageData.items" :items-per-page="props.itemsPerPage"
          ...
          <template v-for="column in props.customStyles" #[`item.${column.key}`]="{ item }" :key="column.key">
            <v-chip
              :style="{ backgroundColor: column.bgColor, color: column.textColor, height: '22px', fontSize: '12px' }"
              class="font-weight-bold">
              <v-icon :style="{ backgroundColor: props.customIcon, color: props.customIcon, size: props.customIcon }"></v-icon>
              {{ item[column.key] }}
            </v-chip>
          </template>
        </v-data-table>
